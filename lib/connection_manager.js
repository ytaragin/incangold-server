...
class ConnectionManager {
    ...
    async postMessage(ConnectionId, Data) {
        try {
            ...
            let postReturn = await this.apiGW.postToConnection({ ConnectionId, Data}).promise();
            console.log(`Post returned: ${postReturn}`);
        } catch (e) {
            if (e.statusCode === 410) {
                console.log(`Found stale connection, deleting ${ConnectionId}`);
                this.disconnectConnection(ConnectionId);
            } else {
                console.log(`Error Sending to ${ConnectionId}`);
                console.log(e);
                throw e;
            }
        }
